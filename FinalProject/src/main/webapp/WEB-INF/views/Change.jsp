...
function NullCheck(){
	if(!document.change.nickname.value){
		alert("닉네임을 입력해주세요.");
		return false;
	}
	else if(!document.change.pw.value){
		alert("비밀번호를 입력해주세요.");
		return false;
	}
	else if(!document.change.tel.value){
		alert("전화번호를 입력해주세요.");
		return false;
	}
	else if(!document.change.mail.value){
		alert("메일을 입력해주세요.");
		return false;
	}
}
...
<body>
  ...
  <div class="container">
    ...
    <div class="row">
  		<form action="getLogin" name="change" onsubmit="return LoginNullCheck()">
      ...
      <div id="mypage" class="mypage">
		<section>			
			      <div id="get">
      	<form action="getClientUpdatePw" name="change" onsubmit="return NullCheck()">
			<div id="signup" style="width: 400px; margin-left: 90%">
				<h1>회원 정보 수정</h1>
				<br><br>
				이름<br>
				<%=login.getName()%><br><br>
				<input type="hidden" name="name" value=<%=login.getName()%>>
				닉네임<br>
				<input type="text" name="nickname" value=<%=login.getNickname() %> readonly>&nbsp&nbsp
				<input type="button" value="닉네임중복확인" class="btn btn-primary btn-lg" onclick="window.open('NickName', '_blank', 'width=500px, height=300px');">
				<br><br>
				아이디<br>
				<%=login.getId()%>&nbsp&nbsp
				<input type="hidden" name="id" value=<%=login.getId() %>>
				<br><br>
				비밀번호<br>
				<input type="password" name="pw" pattern="(?=.*\d)(?=.*[~`!@#$%\\^&*()-])(?=.*[a-z])(?=.*[A-Z]).{6,20}" title="특수문자와 숫자를 포함한 6-20자 영문 대소문자로 설정하세요" value=<%=login.getPw() %>>
				<!-- <a href="getClientUpdatePw"><input type="button" value="비밀번호변경" class="btn btn-primary btn-lg" ></a>
				 --><br><br>
				전화번호<br>
				<input type="tel" name="tel" value=<%=login.getTel()%>>
				<!-- <input type="button" value="전화번호변경" class="btn btn-primary btn-lg" onclick="windowpw()">
				 --><br><br>
				메일<br>
				<input type="email" name="mail"value=<%=login.getMail()%>><!-- 
				<input type="button" value="메일변경" class="btn btn-primary btn-lg" onclick=""> -->
				<br><br>
				<input type="submit" value="수정" class="btn btn-primary btn-lg" style="margin-left:60px" onclick="yes()">
			</div>
		</form>
      </div>
