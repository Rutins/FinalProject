...
//로그인 Null 체크
function End(){
	var pwcheck=document.pwcheckform.pw.value;
	if(pwcheck.length<1){
		alert("비밀번호를 입력해주세요.");
		return false;	
	}
	if(pwcheck!=<%=login.getPw()%>){
		alert("올바른 비밀번호를 입력해주세요.");
		return false;
	}
	if(pwcheck==<%=login.getPw()%>){
		var pwcheck2=confirm("정말 탈퇴하시겠습니까?");		
		if(pwcheck2==true){
			alert("탈퇴 완료되었습니다.");
		}
		else{
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="end" id="end">
		<center><h1>회원탈퇴</h1></center><br>
		<section>			
			<table>
				<tr><td>회원탈퇴를 진행하시려면 비밀번호를 입력해주세요.<br>
				<form id="end" name="pwcheckform" method="post" action="getDelete" onsubmit="return End()" >
				<input type="password" id="my_pw" name="pw"><br><br>
